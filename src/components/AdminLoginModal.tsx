...
import { Lock, X, KeyRound, ShieldAlert, Check, Delete } from 'lucide-react';
...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  const handleSubmit = (e?: React.FormEvent) => {
    ...
    if (!pin) {
      setErrorMsg(isAr ? 'الرجاء أدخل رمز PIN' : 'Please enter PIN code');
      return;
    }

    const success = onLogin(pin);
    if (!success) {
      setErrorMsg(isAr ? 'رمز PIN غير صحيح. جرب 1234' : 'Incorrect PIN code. Try 1234');
      setPin('');
    } else {
      setPin('');
      setErrorMsg('');
