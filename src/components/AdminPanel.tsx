...
import { X, Plus, Edit2, Trash2, Eye, EyeOff, Save, RefreshCw, Store, ListFilter, Utensils, Check, Image as ImageIcon, Upload, Smartphone } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  const handleSaveItem = () => {
    if (!itemForm.nameAr || !itemForm.price) {
      alert(isAr ? 'يرجى كتابة اسم الطبق والسعر' : 'Please fill in item name and price');
      return;
    }
    ...
    if (editingItem) {
      // Update
      const updated = menuItems.map((item) =>
        item.id === editingItem.id ? ({ ...item, ...itemForm } as MenuItem) : item
      );
      onUpdateMenuItems(updated);
    } else {
      // Create new
      const newItem: MenuItem = {
        ...
        nameAr: itemForm.nameAr || 'طبق جديد',
        nameEn: itemForm.nameEn || itemForm.nameAr || 'New Dish',
        descriptionAr: itemForm.descriptionAr || '',
        descriptionEn: itemForm.descriptionEn || '',
        price: Number(itemForm.price),
        categoryId: itemForm.categoryId || 'grills',
