...
export type WhatsAppTarget = 'auto' | 'standard' | 'business' | 'web';
...
export function openWhatsAppOrder(
  ...
  let whatsappUrl = `https://wa.me/${cleanPhone}?text=${encodedText}`;

  if (target === 'business') {
    if (isMobile) {
      // Universal WhatsApp API protocol link which handles WhatsApp Business seamlessly on mobile OS
