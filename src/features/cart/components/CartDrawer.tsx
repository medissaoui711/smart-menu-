...
import { X, Trash2, Plus, Minus, Send, Utensils, ShoppingBag, Truck, MapPin, User, MessageSquare, AlertCircle, Navigation, Loader2, CheckCircle2, Car, Clock } from 'lucide-react';
import { openWhatsAppOrder, WhatsAppTarget } from '../utils/whatsapp';
...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  const handleSendOrder = (target: WhatsAppTarget = whatsAppTarget) => {
    // Validation checks
    ...
    // Send order via WhatsApp (Adaptive mode passed)
    openWhatsAppOrder(config, items, customer, lang, target);
          ...
          >
            ...
            {items.length > 0 && (
              <div className="p-4 bg-slate-50 border-t border-slate-200 space-y-3">
                ...
                {/* WhatsApp App Type Adaptive Selector */}
                <div className="space-y-2 pt-1">
                  <div className="flex items-center justify-between text-[11px] text-slate-600 font-bold px-1">
                    <span>{isAr ? 'تطبيق الواتساب:' : 'WhatsApp App:'}</span>
                    <div className="flex gap-1 bg-slate-200/80 p-0.5 rounded-lg text-[10px]">
                      <button
                        type="button"
                        onClick={() => setWhatsAppTarget('auto')}
                      ...
                      <button
                        type="button"
                        onClick={() => setWhatsAppTarget('standard')}
                        className={`px-2 py-0.5 rounded-md transition cursor-pointer ${
                          whatsAppTarget === 'standard'
                            ? 'bg-emerald-600 text-white font-extrabold shadow-2xs'
                            : 'text-slate-600 hover:text-slate-900'
                        }`}
                      >
                        {isAr ? 'العادي 🟢' : 'Standard'}
                      </button>
                      <button
                        type="button"
                        onClick={() => setWhatsAppTarget('business')}
                        className={`px-2 py-0.5 rounded-md transition cursor-pointer ${
                  ...
                  {/* Main Send WhatsApp Order Button */}
                  <button
                    onClick={() => handleSendOrder(whatsAppTarget)}
                  ...
                  >
                    ...
                    <span className="text-sm">
                      {whatsAppTarget === 'business'
                        ? (isAr ? 'إرسال عبر واتساب الأعمال 💼' : 'Send via WhatsApp Business 💼')
                        : whatsAppTarget === 'standard'
                        ? (isAr ? 'إرسال عبر واتساب العادي 🟢' : 'Send via Standard WhatsApp 🟢')
                        : (isAr ? 'إرسال الطلب عبر الواتساب مباشرة 🚀' : 'Send Order via WhatsApp Direct 🚀')}
                    </span>
                  </button>
                </div>
              </div>
